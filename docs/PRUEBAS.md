...
## PNG de página completa — versión 0.4.0 (histórico, reemplazado en 0.5.0)
...
## PNG de página completa mediante CDP — versión 0.5.0

- La captura full-page de scroll-and-stitch de 0.4.0 queda reemplazada por `chrome.debugger` y Chrome DevTools Protocol. Se conecta con versión `1.3`, ejecuta `Page.enable`, obtiene `Page.getLayoutMetrics` (prefiere `cssContentSize`, con fallback `contentSize`) y realiza una sola llamada `Page.captureScreenshot` con `captureBeyondViewport: true`, `fromSurface: true`, `format: 'png'` y clip del documento. Vuelve a consultar las medidas y verifica la pestaña activa. El destino debugger se libera en `finally`.
- La salida se calcula con las medidas CSS del documento y escala 1×/2×; tras decodificar se planifica contra las dimensiones reales del bitmap para evitar distorsionar su proporción. Se comprueban 16384 px por lado y 40 MP antes de solicitar el screenshot. Los bitmaps se cierran y fallos de CDP/decode/render ejecutan detach.
- No se usa `Emulation.setDeviceMetricsOverride`, no se desplaza la página ni se ensambla una secuencia de viewports. El modo **Área visible** conserva `captureVisibleTab` y no requiere attach al debugger. La copia y la descarga usan el blob renderizado con la misma escala seleccionada.
- El permiso `debugger` se añade a la lista exacta `activeTab`, `scripting`, `clipboardWrite`, `debugger`; Chrome muestra una advertencia amplia para este permiso. Si DevTools u otro cliente debugger mantiene ocupado el destino, el popup solicita cerrarlo y reintentar.
- Las pruebas de Node usan un fake de `chrome.debugger`: verifican orden de attach/comandos/detach, clip y opciones CDP, dimensiones 1×/2×, fallback de medidas, ausencia de llamadas a `captureVisibleTab` o scripts de scroll, límite previo al screenshot, cambio de pestaña, dimensiones cambiantes y cleanup en errores de CDP/decode/render. También prueban captura visible, blob/copia, nombre y metadatos para descarga. No ejecutan un navegador ni validan el render del protocolo o el comportamiento visual de fixed/sticky.
- [ ] Prueba real pendiente en Chrome: aceptar la advertencia de `debugger`, cargar 0.5.0 y probar página completa 1×/2× con descarga y copia; revisar dimensiones y apariencia, y comprobar fixed/sticky en una página real. El resultado de fixed/sticky queda explícitamente sin validar hasta completar esta prueba.
- El máximo de tres desplazamientos, el ensamblado de cuatro tiles, el intervalo de 500 ms, el rechazo de overflow horizontal y la restauración del scroll descritos arriba pertenecen a la implementación 0.4.0 y ya no aplican a 0.5.0.

## PNG de página completa con viewport CSS personalizado — versión 0.6.0

- Se añade **Viewport: Automático / Personalizado** únicamente al modo **Capturar página completa**. Automático mantiene el comportamiento 0.5.0: no ejecuta comandos `Emulation.*` y captura el documento con CDP. Área visible sigue usando `captureVisibleTab` sin debugger ni emulación.
- En personalizado, antes de `Page.getLayoutMetrics` se ejecuta `Emulation.setDeviceMetricsOverride` con el ancho/alto CSS indicados, `mobile: false` y DPR 1 para salida 1× o DPR 2 para salida 2×. El clip usa el ancho CSS elegido y el mayor entre el alto CSS del viewport y el alto documental medido; el PNG puede por tanto ser más alto que el viewport si el documento continúa.
- Las dimensiones deben ser enteros entre 200 y 16384 CSS px. Se mantiene el límite de exportación de 16384 px por lado y 40 MP sobre la salida final. No se estira el documento para fijarlo a la altura del viewport. La escala determina el tamaño de salida, igual para descargar y copiar, y el nombre conserva el sufijo `-1x.png`/`-2x.png`.
- El modo personalizado puede modificar layout responsive, breakpoints y unidades `vh`; simula otro viewport y no garantiza el layout original. El override se limpia con `Emulation.clearDeviceMetricsOverride` antes del detach tanto en éxito como en error. Los errores de cleanup no sustituyen el error original; detach sigue siendo best-effort.
- La selección de página completa, escala, modo de viewport y dimensiones se persiste en `localStorage` del popup. No se añade el permiso `storage`; la preferencia permanece al cerrar/reabrir el popup y al recargar la extensión en el mismo origen/carpeta.
- `extension/settings.js` valida y carga/guarda la preferencia con almacenamiento inyectable. Los tests prueban valores predeterminados, round-trip, rechazo de valores inválidos y recuperación de configuración corrupta. Tests fake de CDP cubren ausencia de comandos Emulation en automático, orden/DPR/dimensiones del override custom, contenido más alto que el viewport, restauración antes de detach en éxito y error, preservación del error original, invalidación antes de attach, captura visible, escalas, copia/descarga, errores y recursos.
- Versiones de `manifest.json` y `package.json`: 0.6.0. `scripts/check.mjs` exige versión 0.6.0 y mantiene la lista exacta de permisos `activeTab`, `scripting`, `clipboardWrite`, `debugger`; no se agregan permisos.
- [ ] Prueba real pendiente en Chrome: probar 1900 × 3000 CSS personalizado a 1× y 2× en una página cuyo documento exceda el viewport; comprobar ancho, alto completo, DPR/nitidez, cambios responsive y `vh`, descarga/copia y restauración del viewport tras éxito y fallo. La emulación y restauración solo se verifican en Node mediante debugger fake hasta completar esta prueba.
